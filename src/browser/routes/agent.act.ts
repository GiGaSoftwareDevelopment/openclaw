...
import { lookupRefInfo, findRefByRoleInfo } from "../pw-session.js";
import {
  type ActKind,
  isActKind,
  parseClickButton,
...
// ---------------------------------------------------------------------------
// Stale-ref auto-retry helpers (CLAW-5)
// ---------------------------------------------------------------------------
...
/** Returns true if the error looks like a stale/missing ref. */
function isStaleRefError(err: unknown): boolean {
  const msg = err instanceof Error ? err.message : String(err);
  return STALE_REF_PATTERNS.some((pattern) => pattern.test(msg));
}

/** Extract all ref-like strings from an action request body. */
function extractRefsFromBody(body: Record<string, unknown>, kind: ActKind): string[] {
  const refs: string[] = [];
  ...
  if (kind === "fill" && Array.isArray(body.fields)) {
    for (const field of body.fields) {
      if (field && typeof field === "object") {
        const fieldRef = toStringOrEmpty((field as Record<string, unknown>).ref);
        if (fieldRef) refs.push(fieldRef);
      }
    }
  }
  return refs;
  ...
  app.post("/act", async (req, res) => {
    ...
    while (true) {
      try {
        ...
        // CLAW-5: Auto-retry with re-snapshot on stale refs
        if (!isRetry && isStaleRefError(err)) {
          try {
            const tab = await profileCtx.ensureTabAvailable(targetId);
            const cdpUrl = profileCtx.profile.cdpUrl;
            const pw = await requirePwAi(res, `act:${kind}:retry-snapshot`);
            if (!pw) {
              return;
            }

            // Save old ref info BEFORE re-snapshot overwrites the cache
            const refs = extractRefsFromBody(body, kind);
            const oldRefInfos = new Map<string, { role: string; name?: string; nth?: number }>();
            for (const ref of refs) {
              const info = lookupRefInfo({ cdpUrl, targetId: tab.targetId, ref });
              if (info) {
                oldRefInfos.set(ref, info);
              }
            }

            // If we couldn't look up any old refs, we can't remap
            if (oldRefInfos.size === 0) {
              throw new Error("no old ref info available");
            }
            ...
            // Attempt to remap stale refs using saved old info + new snapshot
            const mappings: Record<string, string> = {};
            let allMapped = true;
            for (const [oldRef, info] of oldRefInfos) {
              const newRef = findRefByRoleInfo({
                cdpUrl,
                targetId: tab.targetId,
                role: info.role,
                name: info.name,
            ...
            if (allMapped && Object.keys(mappings).length > 0) {
              // Build new body with remapped refs
              const remapped = { ...body };
              if (remapped.ref && typeof remapped.ref === "string" && mappings[remapped.ref]) {
                remapped.ref = mappings[remapped.ref];
              }
              if (kind === "drag") {
                if (
                  remapped.startRef &&
                  typeof remapped.startRef === "string" &&
                  mappings[remapped.startRef]
                ) {
                  remapped.startRef = mappings[remapped.startRef];
                }
                if (
                  remapped.endRef &&
                  typeof remapped.endRef === "string" &&
                  mappings[remapped.endRef]
                ) {
                  remapped.endRef = mappings[remapped.endRef];
                }
              }
              if (kind === "fill" && Array.isArray(remapped.fields)) {
                remapped.fields = (remapped.fields as Record<string, unknown>[]).map((field) => {
                  if (field && typeof field === "object") {
                    const fieldRef = toStringOrEmpty(field.ref);
                    if (fieldRef && mappings[fieldRef]) {
                      return { ...field, ref: mappings[fieldRef] };
                    }
                  }
                  return field;
                });
              ...
              body = remapped;
              isRetry = true;
              continue; // retry the action with remapped refs
            }
          } catch {
            // If re-snapshot or remapping fails, fall through to original error
          }
